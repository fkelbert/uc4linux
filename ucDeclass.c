...
void ucDeclass_add(pid_t pid, ucDataSet dataSet) {
	...
	if (!(t = calloc(1, sizeof(struct timeval)))) {
		ucDeclass_errorExit("Unable to allocate enough memory.");
	}

	if (gettimeofday(t, NULL) == -1) {
		ucDeclass_errorExit("Unable to get time.");
	}

	dataSetCopy = g_hash_table_new_full(g_int_hash, g_int_equal, free, NULL);

	g_tree_insert(times, t, dataSetCopy);

	// copy each and every entry
	g_hash_table_iter_init(&iter, dataSet);
	while (g_hash_table_iter_next (&iter, &container, NULL)) {

		if (!(containerCopy = calloc(1, sizeof(ucContainerID)))) {
			ucPIP_errorExit("Unable to allocate enough memory");
		}
		*containerCopy = * (ucContainerID*) container;
		g_hash_table_insert(dataSetCopy, containerCopy, NULL);
	}
}
