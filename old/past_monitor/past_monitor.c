...
void unhacking(char** str)
{
		char *tmp1, *tmp2;
		tmp1 = strdup(*str);
		tmp2 = str_replace(tmp1, "OPENPAR", "(");
		if (tmp2)
		  {
		    free(tmp1);
		    tmp1 = tmp2;
		  }
		tmp2 = str_replace(tmp1, "CLOSEPAR", ")");
		if (tmp2)
		  {
		    free(tmp1);
		    tmp1 = tmp2;
		  }
		tmp2 = str_replace(tmp1, "COMMA", ",");
		if (tmp2)
		  {
		    free(tmp1);
		    tmp1 = tmp2;
...
int add_past_monitor(char *id, char *formula, int64_t usec_timestep) {
	...
	if(!new_entry){
		...
		// REPLACE THE HACK TO BYPASS THE WEAK PARSER
		unhacking(&formula);
		plog (LOG_TRACE, "parsed formula[%s]", formula);
			...
			if (!IS_NULL_EVENT) {
				if (new_event->xml_doc!=NULL) {
					//print_element_names(new_event->xml_doc);
				        // UNDO the hack for bypassing the weak parser before calling xpath
				  unhacking(&formula->op1);
