...
void type_error(SoupMessage *msg, GType expected, GValueArray *params, int bad_value)
{
  ...
  soup_xmlrpc_set_fault(msg, SOUP_XMLRPC_FAULT_SERVER_ERROR_INVALID_METHOD_PARAMETERS, "Bad parameter #%d: expected %s, got %s", bad_value
      + 1, g_type_name(expected), g_type_name(G_VALUE_TYPE (&params->values[bad_value])));
}
...
void run_gloop(GMainLoop *gloop)
{
  log_trace("Starting gloop thread");
...
{
  init_glib();
  esfxmlrpc_ptr lxmlrpc=(esfxmlrpc_ptr)mem_alloc(sizeof(esfxmlrpc_t));
  check_null_ptr(lxmlrpc, "Could not allocate memory for XML-RPC server");
  ...
  lxmlrpc->service_name=(char*)mem_calloc(strlen(service_name)+1, sizeof(char));
  ...
  pthread_create(&lxmlrpc->gloop_thread, NULL, (void*)run_gloop, lxmlrpc->gloop);
