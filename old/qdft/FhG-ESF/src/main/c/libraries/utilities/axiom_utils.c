...
unsigned int validateXML(axiom_node_ptr rootNode)
{
  ...
  else
  {
    ...
    xmlSchemaSetParserErrors(pctxt, (xmlSchemaValidityErrorFunc) axiom_log_helper, (xmlSchemaValidityWarningFunc) axiom_log_helper, LOG_AXIOM_UTILS_C_OUTPUT);
    ...
    xmlSchemaSetValidErrors(ctxt, (xmlSchemaValidityErrorFunc) axiom_log_helper, (xmlSchemaValidityWarningFunc) axiom_log_helper, LOG_AXIOM_UTILS_C_OUTPUT);
...
void axiom_log_helper(FILE *fp, unsigned char *msg, ...)
{
  va_list fmtargs;
