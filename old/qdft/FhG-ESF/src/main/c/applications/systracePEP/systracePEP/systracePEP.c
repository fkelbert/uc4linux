 ...
 *      Author: cornelius moucha, Ricardo Neisse, Carolina Lorini
 */

#include "systracePEP.h"
#include "log_systracePEP_c.h"
#include "pdp.h"
...
//
//	// Starting PDP
//	// Look for a port or use the default one
//	unsigned long pdp_xmlport = DEFAULT_PDP_XMLPORT;
//	unsigned long pdp_sockport = DEFAULT_PDP_SOCKPORT;
//	for (i = 1; i < argc; ++i) {
//	  if (!strcmp(argv[i], "-x") || !strcmp(argv[i], "--xmlport")) {
//	    assert(i + 1 < argc);
//	    pdp_xmlport = strtoul(argv[i+1], NULL, 10);
//	    log_debug("User-defined port for the xmlrpc PDP: %d", pdp_xmlport);
//	    ++i;
//	  }
//	  else if (!strcmp(argv[i], "-s") || !strcmp(argv[i], "--sockport")) {
//	    assert(i + 1 < argc);
//	    pdp_sockport = strtoul(argv[i+1], NULL, 10);
//	    log_debug("User-defined port for the socket PDP: %d", pdp_sockport);
//	    ++i;
//	  }
//	}
//	if (pdpStart(pdp_xmlport, pdp_sockport) != R_SUCCESS) {
