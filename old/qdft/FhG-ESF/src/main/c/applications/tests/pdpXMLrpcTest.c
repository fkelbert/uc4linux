...
int main()
{
  ...
  char *response;
  char *data="<?xml version='1.0'?><policySet xmlns='http://www.iese.fhg.de/esf/1.0/enforcementLanguage'><preventiveMechanism name='xx1'><description>xx1</description><timestep amount='1' unit='SECONDS'/><trigger action='open' index='ALL' isTry='true'></trigger><condition><XPathEval>/event/parameter[@name='command']/@value='cat'</XPathEval></condition><authorizationAction><inhibit/></authorizationAction></preventiveMechanism></policySet>";
  ...
  if(!check_xmlrpc(&g_return_value, G_TYPE_STRING, &response))
  {
  ...
  if(!do_xmlrpc(session, service_uri, method2, &g_return_value, G_TYPE_STRING, data2, G_TYPE_INVALID))
  {
    log_error("PDP not available, aborting...");
    exit(1);
  }
