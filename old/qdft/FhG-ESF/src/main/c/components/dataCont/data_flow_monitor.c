...
void qdft_removeFromNode(int sizea, char* cont, int oldSize) { //oldsize is only for ftruncate function, -1 otherwise
  ...
  for (dd = 0; dd < length; dd++) {
    ...
    */
    ...
    lod=lod->next;
    log_debug("next=%x",lod);

  }
	

  lod = dataCont_getDataIn(data_flow_monitor->map, cont);
  length = list_of_data_length(lod);
  q = 0;
  for (dd = 0; dd < length; dd++) {
    log_info("checking data %d/%d... q=%d",dd+1,length,q);
    current_data = lod->data->data_ID;
    q += qdftGet_quantity(current_data, cont);
  }
  if (q ==0) {
    log_info("removing container %s",cont);
    dataCont_emptyCont(data_flow_monitor->map, cont);
    dataCont_removeCont(data_flow_monitor->map, cont);
  }
}
...
void qdft_transfNode(int sizea, char* source, char* dest) {
  ...
  for (dd = 0; dd < length; dd++) {
    ...
    log_info ("transfering, data=%s, size=%d, source=%s, dest=%s, length=%d",current_data, sizea,source, dest,length);
    /*int datanode = data2int(current_data);
...
void setInitData(char * d, int * q, void * userdata){ userdata = userdata; log_trace("setinitdata %s (%d)",d,*q); qdftSet_data_amount(d, *q);}
...
unsigned int data_flow_monitor_update(data_flow_monitor_ptr data_flow_monitor,
  ...
  char* OLDSIZE_STR = event_get_param_value_by_name_string(event, "oldSize");

  char* PEP_type=event_get_param_value_by_name_string(event, "PEP_type");    //PEP level (OS, FFOX, DB). So far, empty for FFOX and DB.
...
***************************/
  ...
  char* LIST_OF_INIT_DATA_SIZE = event_get_param_value_by_name_string(event, "init_d_size");
  ...
  /* DB EVENTS parameters
***************************/

  char* DEST = event_get_param_value_by_name_string(event, "dst");
  ...
  if (event != NULL) {
    ...
    if (syscall!=-1){
      ...
      if (PEP_type==NULL){
	PEP_type=strdup("FFOX");
      }
    }
  }

  log_error ("PEP_type=%s",PEP_type);
  deny_because_of_IF = 0;
  ...
  if (strcmp(PEP_type, "OS")==0){
    ...
    case 197: /* -- mmap --
      ...
      event_log("processing mmap-syscall with event: ", event);
  ...
  } else if (strcmp(PEP_type,"FFOX")==0){
    ...
    // -- System call not recognized == FFOX action --
	...
	exit;
      ...
      data_size_arr=parseIntPar (LIST_OF_INIT_DATA_SIZE, &len_arr); //TODO:extend cont-data to 1-n (now 1-1)
      if (len!=len_arr){
	log_error ("[LOAD]len_size=%d != data_size_arr=%d. Break! (Containers not added)",len,len_arr);
	exit;
      }
	...
	if (ind==NULL){
	  ...
	  *b=data_size_arr[i];
	  ...
	  //qdftSet_data_amount(data_arr[i], data_size_arr[i]);
	} else {
	  if (data_size_arr[i] > *ind){
	    *ind=data_size_arr[i];
	    //qdftSet_data_amount(data_arr[i], data_size_arr[i]);
	  }
	}
      ...
      g_hash_table_foreach(maxSizeInitData, (GHFunc)&setInitData, NULL);
	...
	log_info(" * %s [%d][Tq=%d] -----> %s", par_arr[i], sizea_arr[i], data_size_arr[i], data_arr[i]);
    ...
    //SIZEA PASSED AS STRING
		
    if (SIZEA_STR)
      SIZEA = strtol(SIZEA_STR,&end,10);
    if (OLDSIZE_STR)
      OLDSIZE = strtol(OLDSIZE_STR,&end,10);
	
    /* 	if (strcmp(ACTION, "AtoB")==0){ */
    /*  */
    /* 		//assumes only one container as source */
    /*  */
    /* 		log_info("\n DB: Transfer %d units of data from %s to %s \n",SIZEA,TARGET,DEST); */
    /* 		 */
    /* 		lod= dataCont_getDataIn(dc, TARGET); */
    /* 		qdft_transfNode(SIZEA, TARGET, DEST); */
    /* 		 */
    /* 		dataCont_addLodCont(dc, lod, DEST); */
    /* 		 */
    /* 	}	else */ if	(strcmp(ACTION, "removeFrom")==0){
		
      log_info("\n DB: Remove %d units of data from %s (oldsize=%x)\n",SIZEA,TARGET,OLDSIZE);
			
      qdft_removeFromNode(SIZEA, TARGET, OLDSIZE);
		
    }	else if	(strcmp(ACTION, "AtoB")==0){
		
      par_arr=parseStringPar (TARGET, &len);
			
      log_info("\n DB: Move at most %d units of data from %s (%d) to %s \n",SIZEA,TARGET, len, DEST);
			
      char* tmpcont="TMPCONT";
      dataCont_newCont(dc, tmpcont);
			
      for (i=0; i<len; i++){
	lod= dataCont_getDataIn(dc, par_arr[i]);
	dataCont_addLodCont(dc,lod, tmpcont);

	qdft_transfNode(SIZEA, par_arr[i], tmpcont);
      }
						
      lod= dataCont_getDataIn(dc, tmpcont);
      dataCont_addLodCont(dc,lod, DEST);
      qdft_transfNode(SIZEA, tmpcont, DEST);
      qdft_print();

      /* dataCont_emptyCont(dc, tmpcont); */
      /* dataCont_removeCont(dc, tmpcont); */
		
      long dunit = -1;
      char* did = NULL;
      dunit = qdft_checkViolations(&did);
      log_info("dfmonitor. dunit=%d, did=%s",dunit,((did==NULL)? "null": did));

      if (dunit != -1) {
	int dd;
	int q;
	char* current_data;
	list_of_data_ptr lod = dataCont_getDataIn(dc, "t5" );
				
	int length = list_of_data_length(lod);

	printf("Sending %d units of data %s to TABLE-DEST --> forbidden! \n", SIZEA, did);
	deny_because_of_IF = 1;
	for (dd = 0; dd < length; dd++) {
	  current_data = lod->data->data_ID;
	  if (current_data == NULL) {
      ...
      qdft_removeFromNode(INF, tmpcont, -1);
      qdft_print();
    ...
    log_error("PEP_type <%s> not found");
  ...
  if ((nParams > 0) && ((!(strcmp(event->action_desc->action_name, "mmap") == 0) || (strcmp(FDSRC, "-1") != 0))))
    dataCont_print(dc, 2);
...
char* IF_initCont(char* cont, char* qod) {
  ...
  if (gl_data2check == 0){
    ...
    log_debug ("gl_data2check=%s",gl_data2check);
  }
