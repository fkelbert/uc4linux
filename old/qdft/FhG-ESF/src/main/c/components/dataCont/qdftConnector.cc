...
{
	switch (mode){
		case INIT_DATA_MODE_UNKNOWN:
			return strdup(qdftmanager.dm.new_data(q,qdft::unknown).c_str());
		case INIT_DATA_MODE_MAX:
			return strdup(qdftmanager.dm.new_data(q,qdft::max).c_str());
		case INIT_DATA_MODE_LAST:
			return strdup(qdftmanager.dm.new_data(q,qdft::last).c_str());
		default:
			throw std::invalid_argument("new data: wrong mode");
	}
}
...
void qdftSet_data_amount(const char* d, size_t q){
	try {
		...
		switch (INIT_DATA_MODE){
		case INIT_DATA_MODE_UNKNOWN:
			qdftmanager.dm.new_data(q, qdft::unknown, d);
			break;
		case INIT_DATA_MODE_MAX:
			qdftmanager.dm.new_data(q, qdft::max, d);
			break;
		case INIT_DATA_MODE_LAST:
			qdftmanager.dm.new_data(q, qdft::last, d);
			break;
		default:
			throw std::invalid_argument("set data: wrong mode");
		}
	}
}
