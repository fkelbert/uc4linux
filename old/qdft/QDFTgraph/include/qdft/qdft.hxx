// Copyright (C) 2011 Johan Oudinet <oudinet@kit.edu>
//  
...
namespace qdft {
  ...
  template <class Graph>
	...
	// Set the last transfered edge value to its previous value.
  template <class Graph>
	void
	data_managers<Graph>::revert_last_transfer (const dname_type& d)
	{
		typename data_managers_t::iterator it = dm_.find (d);
		if (it == dm_.end ())
			{
		...
		it->second->revert_last_transfer ();
	}
