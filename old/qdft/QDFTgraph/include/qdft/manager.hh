...
namespace qdft {
  ...
  private:
    ...
    detail::simple_counter	next_id_; // Used to maintain the vertex indexes
		bool has_just_transfered_;
		edge_descriptor olde_;
		quantity_type oldq_;
		...
		/** 
		 * Try to remove vertex u and continues recursively
		 * 
		 * @param u the vertex to remove
		 */
		void remove_vertex_rec_ (vertex_descriptor u);
