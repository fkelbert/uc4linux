	...
	switch (code) {
		...
		case SYS_clone:
			...
			if (eax > 0){
				...
				/* Inherit the executed command from its parent */
				create_command(eax,get_command_value(pid));
				change_command_value(eax,get_command_value(pid));
				/* Just in case PTRACE_SETOPTIONS do not work */
				returnCode = ptrace(PTRACE_ATTACH, eax,NULL, NULL);
			}
			break;
		case SYS_kill:
...
event_ptr processStandardParameters(int pid, bool inCall, int code){
	...
	/* Set the syscall name */
	sprintf(cflags,"%s", DFM_eventnames[code]);
	eventAddParamString(event, "syscall", cflags);

	/* Set the command name */
	eventAddParamString(event, "command", get_command_value(pid));
...
//		case SYS_connect:
	...
	case SYS_execve:
		if (code==SYS_execve){
			...
			if (firstcall == 1){
				p_filename = get_command_value(pid);
				size = 0;
			}else{
				p_filename = (char *)calloc(DEFAULT_LENGTH+1, sizeof(char));
				/* The filename is in the first argument of the call */
				getString(pid,regs.ebx,p_filename);
				change_command_value(pid,p_filename);
				stat(filename, &st);
				size = st.st_size;
			}
...
unsigned int store_fd_filename_mapping(int pid, int fd, char *filename)
{
  ...
  if(lfdtable==NULL){
	  ...
	  /* Now we store the file descriptor that we wanted */
...
char* get_file_name(int pid, int fd)
{
  ...
  if(lfdtable==NULL){
	  ...
	  /* Look up the relationship in the hash table
	   * just in case this is stdin, stdout or stderr */
	...
	/* The hash table is already created, so just try to
	 * get the filename */
    char *filename=g_hash_table_lookup(lfdtable, &fd);
    if(filename!=NULL) log_debug("%s - returning [%s]\n", __func__, filename);
    else log_debug("%s - returning NULL", __func__);
...
unsigned int initializePidTable()
{
  ...
  commandtable=g_hash_table_new(g_int_hash, g_int_equal);
...
/*
 * Stores a new pid - Calling command relation in a hash table.
 *
 * @param command The executing command of the process.
 * @param pid Pid of the process we are referring to.
 *
 * @return Integer representing the success of the function.
 */
...
unsigned int create_command(int pid, char *command){
	  char *oldcommand = g_hash_table_lookup(commandtable, &pid);
	  if(oldcommand==NULL){
	      int *y=(int*)memAlloc(sizeof(int));
	      *y=pid;
	      g_hash_table_insert(userpidtable, y, command);
	  }
	  return R_SUCCESS;
}

/*
 * Substitutes the command of a process in a hash table.
 *
 * @param pid Pid of the process we are referring to.
 * @param command Command to substitute the present one.
 *
 * @return Integer representing the success of the function.
 */
unsigned int change_command_value(int pid, char *command){
	g_hash_table_replace (commandtable, &pid, command);
	return R_SUCCESS;
}
/*
...
/*
 * Gets the executing command of a process.
 *
 * @param pid Pid of the process we are referring to.
 *
 * @return Integer representing the success of the function.
...
{
  char *command = g_hash_table_lookup(commandtable, &pid);
  return command;
}
...
unsigned int delete_pid(int pid)
{
  ...
  g_hash_table_remove(commandtable, &pid);
